...
def encode_4sqr(url, cell_size=50):
    # Convert URL to binary with padding
    binary = ''.join(f"{byte:08b}" for byte in url.encode('utf-8'))
    padding = (16 - (len(binary) % 16)) % 16  # Pad to 16-bit boundary
    binary += '0' * padding
    ...
    # Save the image to an in-memory bytes buffer (PNG format)
    buffer = io.BytesIO()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python 4sqr_encoder.py <patientUrl> [cell_size]", file=sys.stderr)
        sys.exit(1)
    patientUrl = sys.argv[1]
    cell_size = int(sys.argv[2]) if len(sys.argv) > 2 else 50
    encoded_base64 = encode_4sqr(patientUrl, cell_size=cell_size)
